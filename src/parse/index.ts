...
const lookup = <I extends Input>(input: I, models: ColorModel<I, AnyColor>[]) => {
...
export const parse = (input: Input): RgbaColor | null => {
  if (typeof input === "string") return lookup<string>(input, colorModels.string);
  if (typeof input === "object") return lookup<InputObject>(input, colorModels.object);
